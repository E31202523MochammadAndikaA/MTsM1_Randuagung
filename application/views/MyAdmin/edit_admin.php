		...
		<div class="p-5">
			<div class="text-center">
				...
				<form class="user" action="<?php echo base_url('Mahasiswa/update'); ?>" method="post">
					...
					<div class="form-group"><input type="password" class="form-control form-control-user" id="pass" name="pass" placeholder="Password Maximal 6 Character" value="<?php echo
					...
					<div class="form-group">
						<select id="grup" class="form-control" name="grup" require>
							...
							<?php foreach ($grup as $baris_grup) { ?>
								<option value="<?php echo $baris_grup->id; ?>" <?php echo $baris->grup == $baris_grup->id ? "selected" : ""; ?>><?php echo $baris_grup->grup; ?></option>
							<?php } ?>
						</select>
					</div>
					<input type="submit" class="btn btn-success px-6 py-2" name="submit" value="Update">
				</form><?php } ?>
			<hr>
